...
def redrawAll(app, canvas):
    ...
    if app.state == 'createDeck':
        ...
        canvas.create_text(
            10, 100, text=f'{app.analysis}', font='Arial 10 bold', anchor=NW, fill='red')
...
def drawClearButton(app, canvas):
    if app.isDeckFull:
        ...
        canvas.create_text(cx, cy, text="Clear",
...
def drawAnalyzeButton(app, canvas):
    if app.isDeckFull:
        ...
        cx, cy = (x0+x1)//2, (y0+y1)//2
        canvas.create_text(cx, cy, text="Analyze!",
        ...
        for col in range(app.deckCols):
            ...
            if ((row, col) == (app.currDeckRow, app.currDeckCol) and
                ...
                canvas.create_rectangle(
                    x0, y0, x1, y1, fill='#CDCDCD', width=2)
    ...
    for row in range(app.cardArrayRows):
        for col in range(app.cardArrayCols):
            ...
            if app.cardImagesMatrix[row][col] != 0:
                ...
                canvas.create_image(
                    x, y, image=ImageTk.PhotoImage(cachedImage))
