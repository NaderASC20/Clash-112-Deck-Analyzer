...
class DeckInfo:
        ...
        for key in self.cardTypesDict:
            ...
            if len(troopList) == 1:
                ...
                self.suggestions.append(
                    f"You're only {key} is {troopList[0]}! Consider adding another {key}."
                )
            elif len(troopList) == 0:
                self.startScore -= 10
                self.suggestions.append(
                    f"You don't have any {key}s in your deck! Consider adding 2 {key}s."
                )
            else:
    ...
    def analyzeElixir(self):
        if self.averageElixir <= 3.0:
            self.startScore -= 0
            self.good.append(
                f"This deck is very easy to cycle with an average elixir of only {self.averageElixir}."
            )
        elif 3.1 <= self.averageElixir <= 4.2:
            self.startScore -= 5
            self.good.append(
                f"This deck is a medium-high elixir deck with an average exlixir of {self.averageElixir}."
            )
        elif self.averageElixir >= 4.3:
            self.startScore -= 10
            self.bad.append(
                f"This deck has a very high elixir cost at an average exlixir of {self.averageElixir}."
            )
            self.suggestions.append(
                f"This deck has a high elixir cost. Consider replacing some high elixir cards with ones that cost less elixir."
            )
